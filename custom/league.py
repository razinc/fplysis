...
class League:
    ...
    async def set_attr(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for page in tqdm(range(1, 203, 1), desc="Parsing top 10k managers      "):
                rank = await league.get_standings(page = page, page_new_entries = 1, phase = 1)
    ...
    async def get_top_10k_ownership(self):
        ...
        top_10k_ownership = dict(sorted(top_10k_ownership.items(), key=lambda x: x[1], reverse=True))
        top_10k_ownership = dict(islice(top_10k_ownership.items(), 50))
        top_10k_ownership = {k: round(v / len(self.top_10k) * 100, 2) for k, v in top_10k_ownership.items()}
