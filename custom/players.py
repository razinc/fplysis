...
class Players():
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for player in tqdm(players, desc = desc):
                ...
                if pd.isna(understat_id) == True or understat_id == "N/A":
                    ...
                    player_grouped_stats = await understat.get_player_grouped_stats(understat_id)
                ...
                for gw in Gameweek.next_n_gws:
                    ...
                    if f"Gameweek {gw}" in gameweeks:
                        ...
                        for i in indices:
                            ...
                            fixtures[gw].append(f"{team_against_short_name} ({where}) ({difficulty})")
                    else:
                        fixtures[gw] = ["Blank GW"]

                stats[fpl_id] = {"web_name": player.web_name,
    ...
    def sort_by_total_pts_prev_n_gw(self):
        return OrderedDict(sorted(self.stats.items(), key = lambda x: getitem(x[1], "total_pts_prev_n_gw"), reverse = True))
    ...
    def sort_by_sum_xg_xa(self):
        return OrderedDict(sorted(self.stats.items(), key = lambda x: getitem(x[1], "sum_xg_xa"), reverse = True))

    def sort_by_xpts(self):
        return OrderedDict(sorted(self.stats.items(), key = lambda x: getitem(x[1], "ep_next"), reverse = True))

    def get_table(self, stats = None, top = None):
        table = PrettyTable()
        header = ["Name", "Team", "Pos", "Price", "xG", "xA"] + [f"GW{gw} Pts" for gw in Gameweek.prev_n_gws] + ["Σ Pts"]+ [f"GW{Gameweek.next_gw} xP"] + [f"GW{gw} Fxt" for gw in Gameweek.next_n_gws] + ["FDA"]
        ...
        for k, v in dict(islice(d.items(), top)).items():
            row = [v["web_name"], v["team_short_name"], v["pos"], v["latest_price"], v["xg"], v["xa"]] + v["pts_prev_n_gw"] + [v["total_pts_prev_n_gw"], v["ep_next"]]
            fixtures = v["fixtures"]
