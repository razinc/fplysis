...
with open("output/analysis_league.csv", "w") as f:
    ...
    f.write(
        "Rank,Team Name,"
        + ",".join([f"GW {str(i)}" for i in range(1, current_gameweek + 1)])
        + "\n"
    )
    for i, manager in enumerate(tqdm(rank, desc="Analysing managers       "), start=1):
        ...
        gameweek_cumulative_points = asyncio.run(
            get_gameweek_cumulative_points(manager_id)
        )
        ...
        f.write(
            ...
            + team_name
            + ","
            + ",".join([str(i) for i in gameweek_points])
            + "\n"
        )
    ...
    for i in rank:
        ...
        f.write(
            team_name
            + ","
            + ",".join([str(i) for i in gameweek_cumulative_points])
            + "\n"
        )
    ...
    goat_team = []
    for gameweek in tqdm(
        range(1, current_gameweek + 1), desc="Analysing weekly top team"
    ):
        top_weekly_points = max(
            [list(i.values())[0]["gameweek_points"][gameweek - 1] for i in rank]
        )
        top_weekly_teams = []
        for manager in rank:
            team_name = list(manager.values())[0]["team_name"]
