...
required_args.add_argument(
    "-id", "--user_id", type=int, help="User ID. Can be obtained in FPL Points's URL."
)
required_args.add_argument(
    "-l",
    "--log_in",
    type=bool,
    help="Enable login. Email & Password must be set in fpl_credentials.py.",
)
...
if args.log_in == True:
    print(
        "There is an issue with login feature. Please use --user_id instead. For more information: https://github.com/amosbastian/fpl/issues/120"
    )
    exit()
...
with open("output/analysis_my_team.txt", "a") as f:
    ...
    not_my_team_ep = sorted(
        not_my_team,
        key=lambda x: list(x.values())[0]["expected_points_next"],
        reverse=True,
    )
